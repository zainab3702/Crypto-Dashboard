  ...
  return (
    <div className="max-sm:w-10/12 w-5/12 max-lg:w-6/12 max-sm:h-6 h-10 flex rounded-md border hover:text-blue-700 hover:border-blue-500">
      <div className="w-full h-full items-center relative flex rounded-md bg-cyan-900 text-white shadow-sm">
        ...
        {isOpen && (
          <div className="min-w-[200px] max-h-[250px] absolute max-sm:top-2 top-7 right-0 z-10 mt-4 origin-top-right rounded-md border border-gray-100 bg-cyan-900 shadow-lg overflow-y-scroll">
            <div>
              {cryptoCurrencies.map((currency, i) => {
                return (
                  <div
                    ...
                    className="flex rounded-lg items-center hover:bg-cyan-900 cursor-pointer gap-1 px-4 py-2 max-sm:text-xs text-base text-white no-underline"
                  >
                    {currency.name}
                  </div>
                );
