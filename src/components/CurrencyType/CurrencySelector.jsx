...
const CurrencySelector = () => {
  ...
  return (
    <div className="lg:w-4/12 inline-flex transition ease-in-out hover:ring-4 ring-blue-400 ring-offset-slate-50 rounded-md ">
      <div className="w-full h-12 lg:h-14 items-center relative flex rounded-md bg-cyan-900 text-white">
        <div
          onClick={toggling}
        ...
        {isOpen && (
          <div className="w-full min-w-[100px] absolute top-11 right-0 z-10 mt-4 origin-top-right rounded-md border border-gray-100 bg-cyan-900 shadow-lg">
            <div>
              {myCurrencies.map((currency, i) => {
                return (
                  <div
                    ...
                    className="block rounded-lg px-4 py-2 text-base text-white no-underline hover:bg-cyan-900 cursor-pointer"
                  >
                    {currency}
                  </div>
                );
